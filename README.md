...
## 4.1 Refresh réel de la page HoYoLAB

L’agent Windows force maintenant un vrai refresh de la page HSR à chaque cycle.

C’est important parce que HoYoLAB peut garder la page ouverte sans renvoyer les nouvelles données `/note` et `/index`. Dans ce cas, l’agent tourne encore, mais les valeurs ne changent pas.

Le refresh fait plusieurs actions :

```text
1. navigation vers la page avec un paramètre anti-cache `_refresh`
2. reload navigateur
3. touche F5 en fallback
```

Dans les logs Windows, tu dois voir régulièrement :

```text
[INFO] Vrai refresh page HSR...
[INFO] Navigation anti-cache...
[INFO] Reload navigateur...
[INFO] F5 fallback...
[NOTE DETECTE]
[INDEX DETECTE]
```

Si tu ne vois pas ces lignes, ton `agent.js` n’est probablement pas à jour.
...
## La page HoYoLAB ne se rafraîchit pas

Symptôme :

```text
L’agent tourne, mais l’énergie ou les stats ne changent pas.
```

Cause probable :

```text
HoYoLAB garde la page en cache ou ne renvoie pas les appels `/note` et `/index`.
```

Fix :

```text
Mettre à jour `windows-agent/agent.js` avec la version qui force un vrai refresh.
```
...
Si ces lignes ne s’affichent jamais, vérifie que tu lances bien le bon fichier :
...
Et si tu utilises la tâche planifiée Windows, vérifie qu’elle pointe bien vers :

```text
C:\hsr-hoyo-agent\start-agent.bat
```
